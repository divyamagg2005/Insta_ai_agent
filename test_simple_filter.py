#!/usr/bin/env python3
"""
Test script to verify simplified filter approach for FFmpeg subtitles
"""
...
def test_simple_filter():
    """Test simplified filter approach for FFmpeg subtitles"""
    va = VideoAgent()
    ...
    print("Testing simplified filter approach for FFmpeg subtitles...")
    ...
    # Test FFmpeg command with simplified filter
    output_path = "output/test_subtitles_simple.mp4"
    cmd = [
        "ffmpeg", "-y",
        "-i", test_video,
        "-vf", filter_str,
    ...
    print("Running FFmpeg command with simplified filter...")
    print(f"Command: {' '.join(cmd[:4])} [filter] {' '.join(cmd[5:])}")
    print()
    
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
        print("✅ FFmpeg subtitle command with simplified filter completed successfully!")
        print(f"Output video: {output_path}")
        return True
    except subprocess.CalledProcessError as e:
        print("❌ FFmpeg subtitle command with simplified filter failed!")
